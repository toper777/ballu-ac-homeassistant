...
- **Миграция v1 → v2** (`async_migrate_entry`): только замораживает старые ID, без сети.
  Должна терпеть записи **без данных**: «игнорированные» обнаружения (`source=ignore`) хранятся
  как записи с пустым `data` (на живом HA их было 7 из 10). HA их не мигрирует (`async_setup`
  выходит раньше), но код не должен на них падать.
